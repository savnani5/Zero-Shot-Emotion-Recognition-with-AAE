...
import os
import numpy as np
from utils.computeFeatures import *
...
def load_data(_path, _ftype, coords, joints, cycles=3, test_size=0.1):
    ...
    data,_,_ = scale(data)
    data = common.get_affective_features(np.reshape(data, (data.shape[0], data.shape[1], joints, coords)))[:, :, :30]
    # data = computeFeatures(np.reshape(data, (data.shape[0], data.shape[1], joints, coords)),)[:, :, :30]
