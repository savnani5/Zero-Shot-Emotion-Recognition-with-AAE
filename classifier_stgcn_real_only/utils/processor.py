...
class Processor(object):
    ...
    def generate_predictions(self, data, num_classes, joints, coords):
        ...
        featur = np.zeros((data.shape[0], 64))
        for i, each_data in enumerate(zip(data)):
            ...
            # get label
            with torch.no_grad():
                output_torch, feat_vect = self.model(each_data)
                output[i] = output_torch.detach().cpu().numpy()
                print(output[i],"Final")
                featur[i] = feat_vect.detach().cpu().numpy()
                # print(featur[i].shape)
                labels_pred[i] = np.argmax(output[i])
        return labels_pred, featur
