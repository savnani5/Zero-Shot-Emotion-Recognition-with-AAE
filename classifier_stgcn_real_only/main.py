...
data_path = os.path.join(base_path, '../feature_data/')
ftype = 'ABHI'#'4DCVAEGCN'
coords = 3
joints = 10
cycles = 1
...
parser.add_argument('--num_epoch', type=int, default=60, metavar='NE',
...
for idx in range(labels_pred.shape[0]):
    print('{:d}.\t{:s}'.format(idx, emotions[int(labels_pred[idx])-1]))
out = {}
for lab,vec in enumerate(vecs_pred):
    out[lab] = vec
    # print(vec.shape,"Check Size")
dd.io.save('output.h5',out)
if args.smap:
    pr.smap()
